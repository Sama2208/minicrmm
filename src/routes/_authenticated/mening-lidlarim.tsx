...
import {
  ...
  SOURCE_LABEL, formatDate,
  type LeadStatus, type LeadSource,
} from "@/lib/crm";

export const Route = createFileRoute("/_authenticated/mening-lidlarim")({ component: MeningLidlarimPage });
...
function MeningLidlarimPage() {
  ...
  const filtered = useMemo(() => {
    return (leadsQ.data ?? []).filter((l) => {
      ...
      if (search) {
        ...
        if (!l.full_name.toLowerCase().includes(s) && !(l.phone ?? "").toLowerCase().includes(s)) return false;
      }
      return true;
    });
  ...
  const updateStatus = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["my-leads"] }); toast.success("Status yangilandi"); },
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <Input placeholder="Ism yoki telefon..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8" />
        </div>
      ...
      <div className="rounded-lg border bg-card overflow-hidden">
        <Table>
          ...
          <TableBody>
            {leadsQ.isLoading && <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">Yuklanmoqda...</TableCell></TableRow>}
            {!leadsQ.isLoading && filtered.length === 0 && <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">Sizga biriktirilgan lidlar yo'q</TableCell></TableRow>}
            {filtered.map((l) => (
              <TableRow key={l.id} className="cursor-pointer" onClick={() => setSelectedId(l.id)}>
                ...
                <TableCell onClick={(e) => e.stopPropagation()}>
                  <Select value={l.status} onValueChange={(v) => updateStatus.mutate({ id: l.id, status: v as LeadStatus })}>
                    <SelectTrigger className={`h-8 w-[200px] text-xs font-medium ${STATUS_BADGE[l.status]}`}><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {STATUS_ORDER.map((s) => <SelectItem key={s} value={s}>{STATUS_LABEL[s]}</SelectItem>)}
                    </SelectContent>
                  </Select>
...
function DetailSheet({ lead, onClose }: { lead: Lead | null; onClose: () => void }) {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["my-leads"] }); toast.success("Saqlandi"); onClose(); },
    onError: (e: Error) => toast.error(e.message),
  });
        ...
        {lead && (
          <>
            <SheetHeader>
              ...
              <SheetDescription>
                <span className={`inline-block px-2 py-0.5 rounded text-xs ${STATUS_BADGE[lead.status]}`}>{STATUS_LABEL[lead.status]}</span>
              </SheetDescription>
            </SheetHeader>
            <div className="space-y-4 p-4">
              ...
              <div>
                ...
                <Input type="date" value={appt} onChange={(e) => setAppt(e.target.value)} className="mt-1" />
              </div>
              <div>
                <Label>Izoh</Label>
                <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={5} className="mt-1" />
              </div>
              <Button onClick={() => save.mutate()} disabled={save.isPending} className="w-full bg-emerald-600 hover:bg-emerald-700">Saqlash</Button>
            </div>
          </>
        )}
      </SheetContent>
