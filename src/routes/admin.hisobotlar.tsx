...
function AdminHisobotlarPage() {
  ...
  const stats = useMemo(() => {
    ...
    const byStatus = STATUS_ORDER.map((s) => ({
      name: STATUS_LABEL[s],
      value: list.filter((l) => l.status === s).length,
    }));
    ...
    sourceMap.forEach((v, k) =>
      bySource.push({ name: SOURCE_LABEL[k as LeadSource] ?? k, value: v }),
    );
  ...
  return (
    <AdminShell title="Hisobotlar">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <Card>
          <CardHeader>
            <CardTitle className="text-sm text-slate-500">Jami lidlar</CardTitle>
          </CardHeader>
          <CardContent className="text-3xl font-bold">{stats.total}</CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle className="text-sm text-slate-500">Yangi</CardTitle>
          </CardHeader>
          <CardContent className="text-3xl font-bold text-emerald-600">
            {stats.byStatus.find((s) => s.name === STATUS_LABEL["yangi"])?.value ?? 0}
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle className="text-sm text-slate-500">Manbalar</CardTitle>
          </CardHeader>
          <CardContent className="text-3xl font-bold">{stats.bySource.length}</CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Manba bo'yicha</CardTitle>
          </CardHeader>
          <CardContent style={{ height: 300 }}>
            <ResponsiveContainer>
